...
import com.ink.recode.value.FloatValue
 ...
 * 浮点数值配置组件
 */
class FloatValueComponent(
    value: FloatValue,
    x: Double,
    y: Double,
    width: Double
) : ValueComponent<FloatValue>(value, x, y, width) {
    
    private val floatValue = value
    ...
    override fun draw(canvas: Canvas, scrollOffset: Double) {
        ...
        // 配置名称和值
        val valueText = String.format("%.2f", floatValue.get())
        val displayText = "${floatValue.name}: §7$valueText"
        ...
        // 进度条
        val range = floatValue.range
        val progress = (floatValue.get() - range.start) / (range.endInclusive - range.start)
    ...
    private fun updateValueFromMouse(mouseX: Double, scrollOffset: Double) {
        ...
        val newValue = (range.start + (range.endInclusive - range.start) * progress).toFloat()
        floatValue.set(newValue.coerceIn(range.start.toFloat(), range.endInclusive.toFloat()))
    }
}
